...
;[contract] tc: numV -> numV
;[purpose]: to check input is numV for arithematic expression or not
;[test] (tc (numV 4))
;(tc (num 4))
;(tc '{fun {x} x})
...
(test (tc (numV 4)) (numV 4))
(test/exn (tc (num 4)) "tc: not both numbers")
(test/exn (tc '{fun {x} x}) "tc: not both numbers")
...
(test/exn (run '{+ 4 {fun {x} x}} (mtSub)) "tc: not both numbers")
(test (run '{+ 4 {with {x {fun {x} x}} {x 5} } } (mtSub)) (numV 9))
(test/exn (run '{+ 10 {+ 5 {fun {x} x}}} (mtSub)) "tc: not both numbers")
(test/exn (run '{- {- 10 {+ 10 50}} {fun {x} x}} (mtSub)) "tc: not both numbers")